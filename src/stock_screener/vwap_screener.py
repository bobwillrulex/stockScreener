...
class _AnchoredVwapState:
    ...
    def update(
        self, candle: StoredFourHourCandle, *, reset: bool
    ) -> tuple[float, float]:
        ...
        stdev = (
            sqrt(self.sn / self.cumulative_volume)
            if self.cumulative_volume and self.sn >= 0
            else 0.0
        )
        return vwap, stdev
...
def find_vwap_proximity_hits(
    ...
    *,
    ...
    ordered = sorted(
        candles,
        key=lambda candle: (candle.symbol, candle.trading_date, candle.bucket_index),
    )
    ...
    for candle in ordered:
        ...
        earnings_reset = (
            candle.trading_date in earnings_dates
            and earnings_key not in used_earnings_anchors
        )
        if earnings_reset:
            used_earnings_anchors.add(earnings_key)
        if earnings_reset or earnings_state.active:
            earnings_vwap, earnings_stdev = earnings_state.update(
                candle, reset=earnings_reset
            )
            _append_hit_if_near(
                hits,
                candle,
...
def load_earnings_dates_to_sqlite(
    db_path: str, rows: Iterable[tuple[str, date | str]]
) -> int:
    """Create/update the earnings event table used to reset earnings VWAP."""

    parsed_rows = [
        (symbol.strip().upper(), _date_arg(earnings_date).isoformat())
...
def load_earnings_csv_to_sqlite(db_path: str, csv_path: str) -> int:
    ...
    with open(csv_path, newline="", encoding="utf-8") as handle:
        ...
        if reader.fieldnames is None or not {"symbol", "earnings_date"}.issubset(
            reader.fieldnames
        ):
...
def screen_russell1000_vwap_proximity(
    ...
    *,
    ...
    The scan reuses an existing Russell 1000 component table. If that table is
    missing or empty, it is populated from Wikipedia before loading symbols.
    """

    _ensure_russell1000_components(db_path)
    ...
    with sqlite3.connect(db_path) as connection:
        ...
        if store_results:
            ...
            _store_proximity_symbol_snapshot(connection, hits)
        return hits


def _ensure_russell1000_components(db_path: str) -> None:
    """Load Russell 1000 components only when the local table is unavailable."""
...
def _append_hit_if_near(
    ...
    distance, nearest_field = (
        (low_distance, "low")
        if low_distance <= high_distance
        else (high_distance, "high")
    )
...
def _load_earnings_dates(
    connection: sqlite3.Connection, symbols: list[str]
) -> dict[str, set[date]]:
    _ensure_earnings_table(connection)
    if not symbols:
        return {}
    ...
    for symbol, earnings_date in rows:
        earnings_dates.setdefault(symbol.upper(), set()).add(
...
def _store_proximity_symbol_snapshot(
    connection: sqlite3.Connection, hits: list[VwapProximityHit]
) -> None:
    """Store the current proximity symbol list and symbols newly added by this scan."""

    connection.execute("""
        CREATE TABLE IF NOT EXISTS vwap_proximity_current_symbols (
            symbol TEXT PRIMARY KEY,
            last_seen_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
        )
        """)
    connection.execute("""
        CREATE TABLE IF NOT EXISTS vwap_proximity_new_symbols (
            symbol TEXT PRIMARY KEY,
    ...
    old_symbols = {
        ...
        for row in connection.execute(
            "SELECT symbol FROM vwap_proximity_current_symbols"
        )
    }
    current_symbols = {hit.symbol.upper() for hit in hits}
    new_symbols = sorted(current_symbols - old_symbols)

    connection.execute("DELETE FROM vwap_proximity_current_symbols")
    connection.executemany(
        """
        INSERT INTO vwap_proximity_current_symbols (symbol, last_seen_at)
        VALUES (?, CURRENT_TIMESTAMP)
        """,
        [(symbol,) for symbol in sorted(current_symbols)],
    )

    connection.execute("DELETE FROM vwap_proximity_new_symbols")
    connection.executemany(
        """
        INSERT INTO vwap_proximity_new_symbols (symbol, detected_at)
        VALUES (?, CURRENT_TIMESTAMP)
        """,
...
def main() -> None:
    ...
    load_earnings = subparsers.add_parser(
        "load-earnings", help="Load earnings reset dates from CSV."
    )
    load_earnings.add_argument("db_path", help="SQLite database path.")
    load_earnings.add_argument(
        "csv_path", help="CSV with symbol and earnings_date columns."
    )
    ...
    screen.add_argument(
        ...
        type=float,
        default=0.1,
        help="Max high/low distance in stdev units.",
    )
    screen.add_argument(
        "--only-first-touch",
        action="store_true",
    ...
    screen.add_argument(
        "--symbol-limit", type=int, help="Limit Russell 1000 symbols for smoke tests."
    )
    screen.add_argument(
        "--no-store",
        action="store_true",
        help="Print hits without writing vwap_proximity_hits.",
    )

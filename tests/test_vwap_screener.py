...
def test_screen_russell1000_vwap_proximity_loads_symbols_events_and_stores_hits(tmp_path):
    ...
    with sqlite3.connect(db_path) as connection:
        stored = connection.execute("SELECT symbol, anchor FROM vwap_proximity_hits ORDER BY anchor").fetchall()
